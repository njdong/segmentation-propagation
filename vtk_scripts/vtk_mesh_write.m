function vtk_mesh_write(file, p)
% Write polydata or unstructured_grid to a VTK file (ASCII for compatibility)
% Usage:
%   vtk_mesh_write(file, p)
    ...
    % Write cell types if available
    if isfield(p, 'celltypes')
       fprintf(fid, 'CELL_TYPES %d\n', length(p.celltypes));
       for i = 1:length(p.celltypes)
           fprintf(fid, '%d\n', p.celltypes(i));
       end
       fprintf(fid, '\n');
    end
