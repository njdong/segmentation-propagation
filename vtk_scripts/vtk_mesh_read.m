function p = vtk_mesh_read(file, varargin)
% Read VTK mesh with polydata or unstructured_grid into a struct
% Usage:
%   p = vtk_mesh_read(file, pars)
% Parameters:
...
%   p.point_data(i).data    N x K array of values
    ...
    if ~any(strcmpi(p.hdr.dst, {'UNSTRUCTURED_GRID', 'POLYDATA'}))
        error('Data types other than POLYDATA and UNSTRUCTURED_GRID not supported');
    end
    ...
    while ~feof(fid)
        ...
        elseif strcmp(mode,'intro') && any(strcmpi(key, ...
                {'cells','polygons','vertices','lines','triangle_strips'}))
        ...
        elseif strcmpi(key, 'cell_types')
            % Read the number of cells 
            dat = vtkread(fid, '%d \n', 1);
            n = double(dat{1});
            data_type = 'uint';            

            % Read the cell type data
            T = vtkreaddata(fid, p, n, data_type, pars);
